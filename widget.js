...
(function() {
    ...
    let callbackInput = '';
    let isInSuggestionFlow = false; // Flag to prevent infinite loops when selecting suggestions
    ...
    inputArea.innerHTML = `
        <div style="display:flex;gap:12px;align-items:center;width:100%;">
            <input class="flossy-input" type="text" placeholder="Type your question and I’ll assist you instantly…" 
    ...
    function completeAppointment() {
        ...
        sendToFlosslyAPI(appointmentData, (response) => {
            ...
            if (response.success) {
                ...
                // Don't show suggestions if we're already in a suggestion flow (prevents loops)
                if (!isInSuggestionFlow) {
                    setTimeout(() => {
                        // Show suggestions if available (only if not already in suggestion flow)
                        if (response.suggestions && response.suggestions.length > 0) {
                            ...
                            setTimeout(() => {
                                // Clear the old date/time values from formData so user can re-enter
                                delete formData.preferredDate;
                                delete formData.preferredTime;
                                
                                // Go back to asking for preferred date
                                currentFormStep = botConfig.appointmentFlow.fields.findIndex(f => f.name === 'preferredDate');
                                if (currentFormStep === -1) {
                                    currentFormStep = 0; // Fallback to first field
                                }
                                showNextFormField();
                            }, 1000);
                        }
                    ...
                    // If we're in suggestion flow and got conflict, just ask for different time
                    setTimeout(() => {
                        addBotMessage("Please select a different date and time for your appointment.");
                        
                        // Remove the suggestions div to prevent confusion
                        const suggestionsDiv = messagesContainer.querySelector('.flossy-suggestions');
                        if (suggestionsDiv) {
                        ...
                        setTimeout(() => {
                            ...
                            // Reset the suggestion flow flag
                            isInSuggestionFlow = false;
    ...
    function showAppointmentSuggestions(suggestions, availableSlots) {
        ...
        suggestionsDiv.querySelectorAll('.flossy-suggestion, .flossy-slot').forEach(suggestion => {
            suggestion.addEventListener('click', function() {
                ...
                // Note: Lead is already created when user first provided date/time
                ...
                sendToFlosslyAPI(appointmentData, (response) => {
                    ...
                    if (response.success) {
                        ...
                        // If we're already in suggestion flow or got conflict from a suggestion, don't show suggestions again
                        // Instead, ask user to manually select a different time
                        const dateDisplay = new Date(formattedDate).toLocaleDateString('en-US', { 
                            month: 'short', 
                            day: 'numeric', 
                            year: 'numeric' 
                        });
                        addBotMessage(`⚠️ This time slot (${formattedTime} on ${dateDisplay}) is already booked. Please select a different date and time.`);
                        ...
                        addBotMessage("❌ Sorry, there was an error booking your appointment. Please try again.");
                        setTimeout(() => {
                            addBotMessage("Would you like to try booking again?");
                            showAppointmentOptions();
                        }, 2000);
    ...
    async function handleAIChat(userMessage) {
        ...
        try {
            ...
            hideTypingIndicator();
            addBotMessage("I'm having connection issues. Please try again in a moment.");
